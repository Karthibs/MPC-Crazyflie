...
class DyPMPC:
    ...
    def __init__(self, A, B, N, Q_diag, R_diag):
        ...
        for _ in range(N-1, -1, -1):
            ...
            term1 = np.linalg.inv(R + B.T @ P @ B)
            term2 = B.T @ P @ A
            K_k = term1 @ term2
            
            # P = Q + A^T P A - A^T P B K
            P = Q + A.T @ P @ A - (A.T @ P @ B) @ K_k
    ...
    def compute_action(self, x_current):
        ...
        return self.K_dp @ x_current


class DyPMPCReferencetracking:
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.mpc = DyPMPC(self.A_aug, self.B_aug, int(horizon), Q_vec, np.asarray(R, dtype=float).ravel())
